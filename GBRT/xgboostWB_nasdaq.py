...
def New_preprocessing(TimeSeries):
   Data_df = TimeSeries
   ndx_column = Data_df['NDX']
   Data_df = Data_df.drop('NDX', axis=1)
   Data_df = Data_df.iloc[:, :Number_Of_Features-1]
   Data_df.insert(0, 'NDX', ndx_column)
   sub=Data_df.iloc[:,1:]
   ...
   Normalized_Data_df = pd.DataFrame(np.column_stack([ndx_column, New_sub]), columns=['NDX'] + list(Data_df.columns[1:]))
   #################################################################################################
...
data_path=r'../dataset/nasdaq100_padding.csv'
...
plot_loss(y_true, y_pred, name='nasdaq_')